...
```bash
claude mcp add --transport stdio naxe --scope local -- uv run naxe
```

To also set a database URL (recommended for PostgreSQL):

```bash
claude mcp add --transport stdio naxe --scope local \
