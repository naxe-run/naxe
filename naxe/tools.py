                    ...
                    "tasks": {
                        ...
                        "items": {
                            ...
                            "properties": {
                                ...
                                "approval_gate": {"type": "boolean", "description": "Set to true if this task requires human sign-off after the agent completes its work. Claim and execute the task normally, then call complete_task — the system automatically routes it to awaiting_approval and returns routed_to_approval: true. Use this for tasks involving irreversible actions, external communication, financial operations, or anything needing human review before the result is accepted."},
                                "human_task": {"type": "boolean", "description": "Set to true for tasks performed entirely by a human, not an agent. Human tasks are never claimable by agents. When all dependencies complete, the task auto-transitions to awaiting_approval. A human confirms completion via approve_task or rejects via reject_task. Use approval_gate for agent-executed tasks that need human sign-off; use human_task for tasks the agent will not perform at all."},
